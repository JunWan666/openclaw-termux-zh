...
function printBanner() {
  console.log(`
鈺斺晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺?鈺?    OpenClaw-Termux v${VERSION}              鈺?鈺?    AI Gateway for Android                鈺?鈺氣晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺?`);
}
...
Commands:
  ...
  Any other command is passed directly to openclaw in proot:
    openclawx onboarding      鈫?openclaw onboarding
    openclawx gateway -v      鈫?openclaw gateway -v
    openclawx doctor          鈫?openclaw doctor
    openclawx <anything>      鈫?openclaw <anything>
...
async function runSetup() {
  ...
  console.log('This will install: proot-distro 鈫?Ubuntu 鈫?Node.js 22 鈫?OpenClaw\n');
  ...
  if (!status.proot) {
    ...
    console.log('  鉁?proot-distro installed');
  }
  console.log('');
  ...
  if (!status.ubuntu) {
    ...
    console.log('  鉁?Ubuntu installed');
  }
  console.log('');
  ...
  if (!status.openClawInProot) {
    ...
    console.log('  鉁?OpenClaw already installed in proot');
  }
  console.log('');
  ...
  // Done
  console.log('鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺?);
  console.log('Setup complete!');
  console.log('');
  console.log('Next steps:');
  console.log('  1. Run onboarding: openclawx onboarding');
  console.log('     鈫?Select "Loopback (127.0.0.1)" when asked!');
...
function showStatus() {
  ...
  console.log('Termux:');
  console.log(`  proot-distro:     ${status.proot ? '鉁?installed' : '鉁?missing'}`);
  console.log(`  Ubuntu (proot):   ${status.ubuntu ? '鉁?installed' : '鉁?not installed'}`);
  console.log('');

  if (status.ubuntu) {
    console.log('Inside Ubuntu:');
    console.log(`  OpenClaw:         ${status.openClawInProot ? '鉁?installed' : '鉁?not installed'}`);
    console.log(`  Bionic Bypass:    ${status.bionicBypassInProot ? '鉁?configured' : '鉁?not configured'}`);
    console.log('');
  }

  if (status.proot && status.ubuntu && status.openClawInProot) {
    console.log('Status: 鉁?Ready to run!');
    console.log('');
    ...
    console.log('Status: 鉁?Setup incomplete');
...
function startGateway() {
  ...
  const frames = ['鉅?, '鉅?, '鉅?, '鉅?, '鉅?, '鉅?, '鉅?, '鉅?, '鉅?, '鉅?];
  let i = 0;
  let started = false;
  ...
  const checkDashboard = setInterval(async () => {
    ...
    try {
      ...
      if (response.ok || response.status < 500) {
        ...
        console.log('鉁?OpenClaw gateway started!\n');
        console.log(`Dashboard: ${DASHBOARD_URL}`);
        console.log('Press Ctrl+C to stop\n');
        console.log('鈹€'.repeat(45) + '\n');
      }
